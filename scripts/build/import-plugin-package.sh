...
output_root="${1:-TestResults/import-plugin-package}"
configuration="${2:-Release}"
rids="${IMPORT_PLUGIN_PACKAGE_RIDS:-win-x64 linux-x64 osx-arm64}"
...
plugin_dir="$package_root/plugins/import"
manifest_path="$plugin_dir/plugin.yaml"

rm -rf "$package_root"
mkdir -p "$plugin_dir" "$package_root/.bukit"
cp plugins/Bukit.Plugin.Import/examples/minimal/.bukit/plugins.yaml "$package_root/.bukit/plugins.yaml"
cp plugins/Bukit.Plugin.Import/examples/minimal/plugins/import/plugin.yaml "$manifest_path"

entry_for_rid() {
  case "$1" in
    win-*) printf 'bin/%s/bukit-plugin-import.exe\n' "$1" ;;
    *) printf 'bin/%s/bukit-plugin-import\n' "$1" ;;
  esac
}
...
for rid in $rids; do
  ...
  dotnet publish plugins/Bukit.Plugin.Import/Bukit.Plugin.Import.csproj \
    -c "$configuration" \
    -r "$rid" \
    --self-contained true \
...
echo "Import plugin package OK: $package_root"
