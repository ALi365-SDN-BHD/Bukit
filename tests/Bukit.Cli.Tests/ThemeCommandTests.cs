...
public sealed class ThemeCommandTests : IDisposable
{
    ...
    public async Task CreateAsync_CreatesStarterBasedTheme()
    {
        ...
        Assert.True(File.Exists(Path.Combine(themeRoot, "layouts", "partials", "analytics.html")));
        ...
        Assert.Contains("name=\"twitter:card\"", seoPartial, StringComparison.Ordinal);
        Assert.Contains("application/ld+json", seoPartial, StringComparison.Ordinal);
        var analyticsPartial = await File.ReadAllTextAsync(Path.Combine(themeRoot, "layouts", "partials", "analytics.html"));
        Assert.Contains("site.analytics.google_analytics_id", analyticsPartial, StringComparison.Ordinal);
        Assert.Contains("googletagmanager.com/gtag/js", analyticsPartial, StringComparison.Ordinal);
