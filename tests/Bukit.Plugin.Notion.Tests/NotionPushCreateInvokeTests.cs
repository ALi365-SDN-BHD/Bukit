...
public sealed class NotionPushCreateInvokeTests : IDisposable
{
    private readonly string _projectRoot;

    public NotionPushCreateInvokeTests()
    {
        _projectRoot = Path.Combine(Path.GetTempPath(), "bukit-notion-plugin-create-" + Guid.NewGuid().ToString("N"));
    ...
    [Fact]
    public void App_PushCreate_MissingTokenFailsWithoutDryRunRequirement()
    {
        Environment.SetEnvironmentVariable("NOTION_TOKEN", null);
        (string seedDir, string mapPath) = WriteValidHandoff();
        PluginInvokeRequest request = CreatePushCreateRequest(seedDir, mapPath);
    ...
    [Fact]
    public void App_PushCreate_WithDryRunFalse_MissingTokenFailsWithoutDryRunRequirement()
    {
        Environment.SetEnvironmentVariable("NOTION_TOKEN", null);
        (string seedDir, string mapPath) = WriteValidHandoff();
        PluginInvokeRequest request = CreatePushCreateRequest(seedDir, mapPath, dryRun: false);
...
""");
    ...
    private PluginInvokeRequest CreatePushCreateRequest(string seedDir, string mapPath, bool? dryRun = null)
    {
