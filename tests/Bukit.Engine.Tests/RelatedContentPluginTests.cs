...
public sealed class RelatedContentPluginTests
{
    ...
    public void DerivePages_SkipsArchiveAndPaginationItems()
    {
        ...
        var fields = meta.ToDictionary(kv => kv.Key, kv => new ContentField("test", kv.Value), StringComparer.OrdinalIgnoreCase);
        var archiveItem = new ContentItem("blog-archive-2024", "Archive", "a", DateTimeOffset.UtcNow, "<p>x</p>", fields);
        var pageItem = new ContentItem("blog-page-2", "Page 2", "p2", DateTimeOffset.UtcNow, "<p>x</p>", fields);
