...
#if !SCRIBAN_NO_ASYNC
            IsAwaitable = typeof(Task).IsAssignableFrom(method.ReturnType)
...
#if !SCRIBAN_NO_ASYNC
        [UnconditionalSuppressMessage("Trimming", "IL2075", Justification = "Fallback for unknown async return types; builtin functions are synchronous")]
        protected async ValueTask<object> ConfigureAwait(object result)
        {
            switch (result)
            {
                ...
                case ValueTask<object> vtObj:
                    return await vtObj.ConfigureAwait(false);
            }
            ...
            throw new NotSupportedException($"Unsupported awaitable type: {result?.GetType()}");
        }
#endif
