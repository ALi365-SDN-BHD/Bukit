...
- production `catch {}` blocks replaced with explicit, observable cleanup handling
...
4. Empty production `catch {}` blocks must be replaced by explicit handling:
...
- `rg -n "catch\\s*\\{\\s*\\}" src -g '*.cs' -g '!bin' -g '!obj'`
