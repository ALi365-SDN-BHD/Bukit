...
public static class NotionPluginApp
{
    public static string Handle(string input)
    {
        ...
        return type switch
        {
            PluginProtocolConstants.Handshake => Serialize(NotionPluginManifestProvider.CreateHandshakeResponse(requestId, ReadHostPlatform(root))),
            PluginProtocolConstants.Manifest => Serialize(NotionPluginManifestProvider.CreateManifestResponse(requestId)),
            PluginProtocolConstants.Invoke => Serialize(NotionPluginInvoker.Invoke(ReadInvokeRequest(root))),
            _ => Serialize(new PluginResponseEnvelope(
                Type: "errorResponse",
                Protocol: PluginProtocolConstants.ProtocolVersion,
                RequestId: requestId,
                Success: false,
                Error: new PluginError("plugin.notion.unknownRequest", $"Unsupported request type: {type}")))
        };
    }
