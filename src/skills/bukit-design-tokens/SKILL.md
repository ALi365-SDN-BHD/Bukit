...
| Tailwind, Bootstrap, or another framework overrides styling | External framework CSS loads after the theme CSS | Load external frameworks first and load the theme's `style.css` last so token-backed theme styles win the cascade |
