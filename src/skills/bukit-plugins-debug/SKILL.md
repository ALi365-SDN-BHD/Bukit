...
description: Use when using bukit and plugins do not take effect or behave unexpectedly, build output does not meet expectations, incremental builds behave incorrectly, when developing custom bukit plugins, or diagnosing build performance issues
...
public class HelloPlugin : IDerivePagesPlugin
{
    ...
    public IEnumerable<RoutedContentDocument> DerivePages(BuildContext context)
    {
        var fields = ContentFieldReader.ToFieldMap(
            new Dictionary<string, object> { ["type"] = "page" });
        var publishedAt = DateTimeOffset.UtcNow;
        var record = new ContentRecord(
            new ContentIdentity("hello", "hello-plugin", "hello-plugin", "page", "published"),
            new ContentPresentation("Hello from Plugin", null, "<p>Generated by plugin</p>", "und", []),
            new ContentClassification("page", "page", [], []),
            new ContentOwnership(null, null, null, null),
            new ContentLifecycle(publishedAt, null, null, null),
            new ProvenanceRecord("plugin", null, [], [], null),
        ...
        var document = new ContentDocument(
            record,
            new ContentBodyRef(Html: "<p>Generated by plugin</p>"),
            customFields: fields);

        var route = RouteGenerator.Generate(document, context.Config.Site.OutputPathEncoding);

        yield return new RoutedContentDocument(document, route, publishedAt);
    }
}
```
