...
internal static partial class TemplateStaticAnalysisService
{
    ...
    private sealed class Analyzer
    {
        ...
        {
            var normalized = NormalizePath(templateRelativePath);
        ...
        private static bool TryParseLayoutLine(string line, out string layoutTemplateRelativePath)
        {
            ...
            if (TryParseDirective("{%", "%}", line, out var inner) || TryParseDirective("{{", "}}", line, out inner))
            {
                ...
                {
                    layoutTemplateRelativePath = NormalizePath(path);
                    return true;
                }
            }
        ...
        private static string NormalizePath(string templateRelativePath)
