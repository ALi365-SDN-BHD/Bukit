...
internal static class NotionContentPropertyParser
{
    ...
    internal static string? ExtractTitle(JsonElement properties, NotionPropertyMapConfig? propertyMap = null)
    {
        ...
        if (NotionContentSource.TryGetPropertyIgnoreCase(properties, fieldName, out var titleProp))
        {
        ...
        {
            var v = prop.Value;
            if (NotionContentSource.GetString(v, "type") == "title")
            {
    ...
    internal static string? ExtractSlug(JsonElement properties, NotionPropertyMapConfig? propertyMap = null)
    {
        ...
        if (!NotionContentSource.TryGetPropertyIgnoreCase(properties, fieldName, out var slugProp))
        {
            return null;
        }

        var type = NotionContentSource.GetString(slugProp, "type");
        if (type == "rich_text" && slugProp.TryGetProperty("rich_text", out var rt))
        {
            var text = NotionContentSource.ExtractPlainText(rt);
            return string.IsNullOrWhiteSpace(text) ? null : text.Trim();
        }

        if (type == "formula" && slugProp.TryGetProperty("formula", out var f) && f.ValueKind == JsonValueKind.Object)
        {
            var value = NotionContentSource.GetString(f, "string");
    ...
    internal static string? ExtractType(JsonElement properties, NotionPropertyMapConfig? propertyMap = null)
    {
        ...
        if (!NotionContentSource.TryGetPropertyIgnoreCase(properties, fieldName, out var typeProp))
        {
            return null;
        }

        var t = NotionContentSource.GetString(typeProp, "type");
        if (t == "select" && typeProp.TryGetProperty("select", out var sel) && sel.ValueKind == JsonValueKind.Object)
        {
            return NotionContentSource.GetString(sel, "name");
        }
        ...
        if (t == "multi_select" && typeProp.TryGetProperty("multi_select", out var ms) && ms.ValueKind == JsonValueKind.Array)
        {
            ...
            {
                return NotionContentSource.GetString(first, "name");
            }
        }
    ...
    internal static string? ExtractCollection(JsonElement properties, NotionPropertyMapConfig? propertyMap = null)
    {
        ...
        if (!NotionContentSource.TryGetPropertyIgnoreCase(properties, fieldName, out var collectionProp))
        {
            return null;
        }

        var notionType = NotionContentSource.GetString(collectionProp, "type");
    ...
    internal static DateTimeOffset? ExtractPublishAt(JsonElement properties, NotionPropertyMapConfig? propertyMap = null)
    {
        ...
        if (NotionContentSource.TryGetPropertyIgnoreCase(properties, primaryField, out var dateProp))
        {
    ...
    {
        var type = NotionContentSource.GetString(prop, "type");
        if (type != "date")
        {
            return null;
        }
        ...
        var start = NotionContentSource.GetString(dateObj, "start");
        ...
        NotionPropertyMapConfig? propertyMap)
    {
        ...
        if (!string.IsNullOrWhiteSpace(propertyMap.SeoTitle) &&
            NotionContentSource.TryGetPropertyIgnoreCase(properties, propertyMap.SeoTitle, out var seoTitleProp))
        {
        ...
        if (!string.IsNullOrWhiteSpace(propertyMap.SeoDescription) &&
            NotionContentSource.TryGetPropertyIgnoreCase(properties, propertyMap.SeoDescription, out var seoDescProp))
        {
        ...
        if (!string.IsNullOrWhiteSpace(propertyMap.SeoImage) &&
            NotionContentSource.TryGetPropertyIgnoreCase(properties, propertyMap.SeoImage, out var seoImageProp))
        {
        ...
        if (!string.IsNullOrWhiteSpace(propertyMap.Canonical) &&
            NotionContentSource.TryGetPropertyIgnoreCase(properties, propertyMap.Canonical, out var canonicalProp))
        {
        ...
        string pageId)
    {
        ...
        if (!string.IsNullOrWhiteSpace(propertyMap.EntitiesJson) &&
            NotionContentSource.TryGetPropertyIgnoreCase(properties, propertyMap.EntitiesJson, out var entitiesProperty))
        {
        ...
        bool wrapTextInList = false)
    {
        if (string.IsNullOrWhiteSpace(propertyName) ||
            !NotionContentSource.TryGetPropertyIgnoreCase(properties, propertyName, out var property))
        {
            return;
        }
    ...
    private static void ValidateMappedPropertyType(
        ...
        IReadOnlyList<string> allowedTypes)
    {
        var actualType = NotionContentSource.GetString(property, "type") ?? "<missing>";
