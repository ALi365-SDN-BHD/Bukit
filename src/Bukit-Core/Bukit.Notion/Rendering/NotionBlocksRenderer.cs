...
public sealed class NotionBlocksRenderer
{
    ...
    public NotionBlocksRenderer(NotionClient client)
        : this(client, NotionBlockRendererRegistry.CreateDefault())
    {
    }

    public NotionBlocksRenderer(NotionClient client, NotionBlockRendererRegistry registry)
    {
    ...
    internal async Task RenderChildrenToBuilderAsync(string blockId, StringBuilder sb, CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            {
                throw new NotionRenderingException("Notion blocks response missing results.");
            }
