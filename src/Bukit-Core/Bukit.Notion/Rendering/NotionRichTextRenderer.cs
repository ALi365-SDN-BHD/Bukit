    ...
    public static string Render(JsonElement richTextArray)
    {
        ...
        foreach (var item in richTextArray.EnumerateArray())
        {
            ...
            if (string.Equals(itemType, "mention", StringComparison.OrdinalIgnoreCase))
            {
                ...
                var safeHref = RenderingSafeUrl.ForLink(rawHref);
                ...
                {
                    var rel = RenderingSafeUrl.IsExternal(safeHref) ? " rel=\"noopener noreferrer\"" : "";
            ...
            var safeHref2 = RenderingSafeUrl.ForLink(rawHref2);
            var annotations = item.TryGetProperty("annotations", out var ann) ? ann : default;

            if (!string.IsNullOrWhiteSpace(safeHref2))
            {
                var rel = RenderingSafeUrl.IsExternal(safeHref2) ? " rel=\"noopener noreferrer\"" : "";
