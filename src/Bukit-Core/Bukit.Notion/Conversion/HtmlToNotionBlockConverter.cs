...
public static class HtmlToNotionBlockConverter
{
    ...
    private static string? GetAttribute(string attrs, string attrName)
    {
        ...
        {
            var candidate = attrs.IndexOf(search, searchFrom, StringComparison.OrdinalIgnoreCase);
            if (candidate < 0)
            {
                return null;
            }

            if (candidate == 0 || char.IsWhiteSpace(attrs[candidate - 1]))
            {
                attributeIndex = candidate;
                break;
            }

            searchFrom = candidate + search.Length;
        }

        var startIdx = attributeIndex + search.Length;
        if (attributeIndex < 0 || startIdx >= attrs.Length)
        {
            return null;
        }
        ...
        List<HtmlTokenizer.HtmlToken> tokens, ref int i, string closeTag)
    {
        ...
        while (i < tokens.Count)
        {
            ...
            {
                if (t.TagName == "code" && sb.Length == 0)
                {
                    var className = GetAttribute(t.Attributes, "class");
