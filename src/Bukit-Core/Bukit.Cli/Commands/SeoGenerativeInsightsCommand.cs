...
internal static partial class SeoGenerativeInsightsCommand
{
    ...
    internal static int Run(CliBoundCommand command)
    {
        try
        {
            ...
            var outputPath = ResolveLocalPath(
                command.GetString("--out") ?? Path.Combine(outputDirectory, ".bukit", GenerativeCitationReportWriter.FileName),
                "output_path_invalid");
            ...
            var datasets = observationPaths
                .Select(path => (path, GenerativeAnswerObservationReader.Read(path)))
                .ToArray();
            ...
            Console.WriteLine(
                $"SEO generative insights: sourceRows={counts.SourceRows} matched={counts.MatchedRows} unmatched={unmatchedCount} ambiguous={ambiguousCount}");
            Console.WriteLine($"SEO generative insights report: {outputPath}");
            Console.WriteLine($"SEO generative insights classification: {classification}");
            return strictJoinFailed ? 1 : 0;
        }
        catch (SeoGenerativeInsightsCommandException exception)
        {
            Console.Error.WriteLine($"SEO generative insights failed: {exception.Code}.");
            return 2;
        }
        ...
        {
            Console.Error.WriteLine($"SEO generative insights failed: {StableDataCode(exception)}.");
            return 2;
        }
        catch (Exception)
        {
            Console.Error.WriteLine("SEO generative insights failed: input_unavailable.");
            return 2;
        }
    }
    ...
    private static string ResolveCanonicalPathIdentity(string path)
    {
        ...
        catch (SeoGenerativeInsightsCommandException)
        {
            throw;
        }
    ...
    private static void WriteReport(
        string outputPath,
        GenerativeCitationReport report,
        string routeMapPath,
        string rulePath,
        IReadOnlyList<string> observationPaths)
    {
        ...
        var stagingRoot = Path.Combine(parent, $".seo-generative-insights-{Guid.NewGuid():N}.tmp");
        try
        {
            Directory.CreateDirectory(parent);
            GenerativeCitationReportWriter.Write(stagingRoot, report);
            var stagedReport = Path.Combine(stagingRoot, ".bukit", GenerativeCitationReportWriter.FileName);
            RejectOutputConflict(outputPath, routeMapPath, rulePath, observationPaths);
    ...
    private static SeoGenerativeInsightsCommandException Failure(string code) => new(code);
    ...
    private sealed class SeoGenerativeInsightsCommandException(string code) : Exception
    {
