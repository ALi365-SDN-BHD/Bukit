...
internal static class DoctorMarkdownChecker
{
    ...
    {
        foreach (var absDir in GetMarkdownDirectories(ctx))
        {
    ...
    private static IReadOnlyList<string> GetMarkdownDirectories(DoctorCommand.DoctorContext ctx)
    {
        return ContentSourceInspector.GetMarkdownDirs(ctx.Config.Content)
            .Select(dir => Path.GetFullPath(Path.Combine(ctx.RootDir, dir)))
