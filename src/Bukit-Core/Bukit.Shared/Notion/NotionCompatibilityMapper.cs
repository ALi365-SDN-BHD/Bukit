...
internal static class NotionCompatibilityMapper
{
    internal static NotionBlock ToLegacy(NewBlocks.NotionBlock block)
        => block switch
        {
            ...
            NewBlocks.ParagraphBlock value => new ParagraphBlock(value.Segments.Select(ToLegacy).ToList()),
            NewBlocks.BulletedListItemBlock value => new BulletedListItemBlock(value.Segments.Select(ToLegacy).ToList()),
            NewBlocks.NumberedListItemBlock value => new NumberedListItemBlock(value.Segments.Select(ToLegacy).ToList()),
            NewBlocks.QuoteBlock value => new QuoteBlock(value.Segments.Select(ToLegacy).ToList()),
            NewBlocks.ImageBlock value => new ImageBlock(value.Url, value.Caption),
            NewBlocks.ToggleBlock value => new ToggleBlock(value.Heading, value.Children.Select(ToLegacy).ToList()),
    ...
    internal static NewBlocks.NotionBlock ToIndependent(NotionBlock block)
        => block switch
        {
            ...
            ParagraphBlock value => new NewBlocks.ParagraphBlock(value.Segments.Select(ToIndependent).ToList()),
            BulletedListItemBlock value => new NewBlocks.BulletedListItemBlock(value.Segments.Select(ToIndependent).ToList()),
            NumberedListItemBlock value => new NewBlocks.NumberedListItemBlock(value.Segments.Select(ToIndependent).ToList()),
            QuoteBlock value => new NewBlocks.QuoteBlock(value.Segments.Select(ToIndependent).ToList()),
            ImageBlock value => new NewBlocks.ImageBlock(value.Url, value.Caption),
            ToggleBlock value => new NewBlocks.ToggleBlock(value.Heading, value.Children.Select(ToIndependent).ToList()),
