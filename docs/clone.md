...
│                                                                 │
└────────────────────────────────────────────────────────────────┘
核心洞察：Bukit 的插件系统（IBukitPlugin + PluginRunner）是为构建期内容变换设计的，它的输入是已加载的内容文档 + BuildContext，输出是派生页面或构建产物。克隆网站到主题是开发期脚手架创建，它的输入是 URL + 浏览器 DOM，输出是 themes/<name>/ 目录。两者的数据模型完全不重叠——强行将克隆塞入插件系统，如同用洗衣机洗盘子：工具本身没问题，但用途不对。
