...
`--content-source` 只决定 seed 审核/推送产物，不决定 build provider。`--build-source markdown` 是默认值，生成本地 markdown draft 并让工程可离线 build/verify；`--build-source notion` 才会生成 Notion build provider 并跳过 `content/*.md` 草稿。`--build-source notion` 只能与 `--content-source notion` 组合使用。
