...
Notes:
- Derived pages use the default template `pages/page.html`
- Configurable through `taxonomy.template` / `taxonomy.indexTemplate` / `taxonomy.termTemplate`
- Supports kind-level overrides: `taxonomy.templates.tags.*` / `taxonomy.templates.categories.*`
- Priority: kind-level index/term > global index/term > kind-level template > global template > default `pages/page.html`
...
Notes:
- Derived pages use the template `pages/pagination.html` first, falling back to `pages/page.html`
...
Notes:
- Derived pages use the default template `pages/page.html`; in v3.0+ it can be customized through `collection.output.archiveDetail.template`
