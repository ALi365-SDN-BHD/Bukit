...
Bukit also writes machine-readable publish projections for each content document: `content/*.json` exposes the canonical content record for integrations, and `content/*.md` exposes a text-first representation for RAG or knowledge ingestion. `agent-manifest.json` is generated by the projection pipeline and lists only indexable documents with their available HTML, semantic HTML, JSON, Markdown, and JSON-LD representations. Publish audit verifies that declared JSON and Markdown representations exist and inventories aggregate outputs such as RSS, Atom, JSON Feed, sitemap, search, llms.txt, robots.txt, and the agent manifest at route level, including whether each route is present in AI/crawler-facing outputs.
