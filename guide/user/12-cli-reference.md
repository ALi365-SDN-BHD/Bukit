...
`import html-demo` writes a theme under `themes/<theme>/`, a site config under `sites/<theme>/site.yaml`, markdown drafts under `sites/<theme>/content/`, seed review files, and `import-report.md`. `--verify` runs `doctor` and `build` against the generated site config.
