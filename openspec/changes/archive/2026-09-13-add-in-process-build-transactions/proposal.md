...
提案历史基线为 main `4e6f80c05de8105e638780241a2af0b51a568bb7`；本次授权实施重新锚定 main `0ee017a8`，保留已有提案修改，不以隔离分支作为实现基线。
...
用户已确认“语言隔离并非必须”，并要求基于 main 整理替代提案。用户随后通过“执行下一步”批准具体合同的本地实施；运行时实现与本地验收已完成，专项复审、限定复审及最终 delta review 均通过；正式 spec 已同步，变更已归档。该结论不包含跨平台、断电、真实 Notion、部署或性能验收。

本提案获准实施，取代 `codex/i18n-worktree-isolation`（审阅 HEAD `93b1d77f`）中的 `add-i18n-worktree-isolation` 作为本次交付目标：
