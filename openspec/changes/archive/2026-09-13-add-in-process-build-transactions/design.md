...
目录资源占有整个子树：跨构建路径相等或存在祖先/后代关系均视为冲突，文件资源与包含它的目录也冲突。`public` 与 `public/en`、`.cache` 与 `.cache/notion`、另一构建输出内的 metrics 都不能同时取得使用权。资源身份按实际文件系统的大小写规则处理，并解析已有祖先的真实路径后拼接尚不存在的路径段；符号链接等别名不得生成独立锁身份，无法安全识别的目标在写入前拒绝。

冲突检查与资源占有必须作为不可分割的登记操作，覆盖同进程及跨进程调用；不得先无锁检查再分别创建目标锁。实现可短暂串行化登记，但不相交资源的构建执行不串行化。同一事务内先归并提交目标：同一资源只提交一次，父缓存包含子缓存时保留父目标并映射子路径，目录内 metrics 跟随目录提交；不能安全归并的重叠目标在开始写入前拒绝。独立只读资源登记读取占有，允许读者共存，但与重叠写者互斥。

### 3. 实际路径与报告身份分离

输出 staging 与正式输出同级；需要修改的缓存 staging 与正式缓存同级。clean 时输出从空目录开始，但正式输出的 marker/路径检查仍须执行；no-clean 时复制旧输出，保留非自有文件。可写缓存复制旧状态并保留非本次拥有的文件，所有受管写入转向暂存位置；独立只读缓存不复制、不交换，也不要求其父目录可写。
...
以下资源是已确认的必需范围，不能仅替换 `ConfigOverrides.CacheDir`：

| 资源 | 当前正式路径来源 | 事务处理 |
| --- | --- | --- |
| 根及各语言 manifest | `ConfigOverrides.CacheDir`，默认项目 `.cache` | 在缓存 staging 读写；持久化输出身份恢复为正式路径 |
| 默认媒体缓存 | 上述缓存根下 `media`；空或默认 `assets/uploads` 的 DownloadDir 使用此位置 | 复用缓存 staging 子目录 |
| 自定义媒体下载目录 | `media.DownloadDir`，公开配置仅接受相对项目根路径；内部解析为绝对路径 | 纳入受管写集、锁与提交；重定向本地下载及后续读取，不改变 `UrlBase`；不可安全暂存则写入前拒绝 |
| Notion 页面及关系缓存 | `notion.CacheDir`，默认项目 `.cache/notion`，不服从 Engine CacheDir | `readwrite` 显式重定向 provider 实际缓存路径；默认嵌套路径随父缓存提交，自定义独立路径单独暂存 |
| Notion 只读/关闭缓存 | `notion.cacheMode=readonly/off` | readonly 不写入、不创建缺失目录，命中及缺失行为保持；off 不产生该缓存资源 |
| metrics JSON 与派生 HTML | `MetricsPath` 及 `Path.ChangeExtension(path, ".html")` | 预先解析两个实际目标，分别检查冲突；位于受管目录内则归并，否则作为独立文件提交 |

只读缓存若位于本事务可写父目录内，读取暂存副本并保持该子树内容不变，不再单独交换。内置 Notion 与媒体写入属于受管行为，不归入自定义 provider 的外部副作用豁免。实施清单必须为每项记录解析入口、读写方式、实际读写调用方、正式/暂存映射及句柄释放点，并核查其余受管缓存和内置插件；不得用本表替代完整写集核查。
